...
async function loadVisitStats() {
  const container = document.getElementById('visitStatsList');
  if (!container) return;
  container.innerHTML = '<div class="loading">加载中...</div>';
  ...
  try {
    ...
    if (!data.ok || !Array.isArray(data.data) || data.data.length === 0) {
      container.innerHTML = '<div class="empty-state">暂无访问数据</div>';
      return;
    }

    const PRESET = ['哔哩哔哩', '抖音', '快手', '小红书', '我不想回答这个问题'];
    const presetCounts = {};
    PRESET.forEach(p => presetCounts[p] = 0);
    const others = [];
    let total = 0;

    data.data.forEach(item => {
      total += item.count;
      if (PRESET.includes(item.source)) {
        presetCounts[item.source] = item.count;
      } else {
        others.push({ source: item.source, count: item.count });
      }
    });

    others.sort((a, b) => b.count - a.count);
    const othersTotal = others.reduce((s, o) => s + o.count, 0);
    const maxCount = Math.max(...Object.values(presetCounts), othersTotal, 1);

    let html = `<div class="visit-stats-total">总访问量：<strong>${total}</strong> 人</div>`;
    html += '<div class="visit-stats-list">';
    ...
    presetItems.forEach(item => {
      const c = presetCounts[item.key];
      const pct = c / maxCount * 100;
      html += `
        <div class="visit-stat-row">
          <span class="vs-name">${item.name}</span>
          <div class="vs-bar-wrap"><div class="vs-bar" style="width:${pct}%"></div></div>
          <span class="vs-count">${c} 人</span>
        </div>
      `;
    });

    const othersPct = othersTotal / maxCount * 100;
    html += `
      <div class="visit-stat-row">
        <span class="vs-name">💡 其它</span>
        <div class="vs-bar-wrap"><div class="vs-bar vs-bar-other" style="width:${othersPct}%"></div></div>
        <span class="vs-count">${othersTotal} 人</span>
      </div>
    `;

    if (others.length > 0) {
      html += '<div class="visit-stat-others">';
      others.forEach(o => {
        html += `<div class="visit-stat-other-item">
          <span class="vso-name">${escapeHtml(o.source)}</span>
          <span class="vso-count">${o.count} 人</span>
        </div>`;
      });
      html += '</div>';
    }

    const skipCount = presetCounts['我不想回答这个问题'];
    const skipPct = skipCount / maxCount * 100;
    html += `
      <div class="visit-stat-row">
        <span class="vs-name">🤐 我不想回答</span>
        <div class="vs-bar-wrap"><div class="vs-bar vs-bar-skip" style="width:${skipPct}%"></div></div>
        <span class="vs-count">${skipCount} 人</span>
      </div>
    `;
