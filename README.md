...
├── blog.html                  卡片2 · 玩家博客（文章列表 / 文章详情 / 评论区 / 点赞 / 举报）
...
一个**标准博客**的样子：文章列表 → 点进去看全文 → 底下评论区。**不需要注册**，填个昵称就能写。

- **三个页面用一个文件搞定**（hash 路由，静态托管也能用，浏览器前进后退正常）：
  - `blog.html` → 文章列表（标题 + 摘要 + 作者 + 时间 + 评论数 + 点赞数 + 「阅读全文 →」）
  - `blog.html#/post/12` → 文章详情（大标题 + 全文 + 评论区）
  - `blog.html#/write` → 写文章（昵称 + 标题 + 正文）
- **标题必填**：主帖没有标题会被后端拒收；**评论**（回复）不需要标题。
- **评论区**只做一层：评论下面不再套评论，避免无限套娃。
- **自己写的文章**可以在详情页直接删掉。靠的是浏览器本地那串随机身份（见下），
...
- **违规内容处理**（三层）：
  ...
  3. **后台处置**：后台 → 卡片管理 → 📝 玩家博客，可以
     ...
     拉黑是按浏览器身份生效的 —— 换个昵称照样发不出来，他名下已有的文章也会一起下线。
