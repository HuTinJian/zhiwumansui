...
| **「到目前一共花了多少钱」这个数字** | `js/common.js` 最上面的 `SITE_COST` | 这个数字现在**只在首页「赞助者荣誉榜」显示这一处**（hero 的赞赏码小条、赞赏码弹窗、新人弹窗都已经不写金额和核对日期了）。要改数字，就改 `amount` 这一行，顺便把 `checkedAt` 改成你核对这天的日期；首页荣誉榜里还有一份**没脚本时的兜底**数字（占位元素，属性名见 `js/common.js` 里 `fillSiteCost` 的选择器），换数字时顺手一起改 |
| **赞赏码放在哪 / 换成别的码** | `images/sponsor-qrcode.png` + `index.html` + `js/common.js` | 换码直接替换那张图。二维码只露一处：首页「🚀 开始逛逛」按钮下面那条（`<button class="hero-sponsor">`，点开是全站同一套大图弹窗）。**新人弹窗里不放图**，只留一句「赞助码在首页」的提示（`js/common.js` 里的 `.preview-notice-sponsor-tip`）—— 顺带省掉首访下载 220KB 二维码的开销 |
| **老访客看不到新版新人弹窗** | `js/common.js` | 那个弹窗每个浏览器只弹一次。想让它对所有人再弹一遍，把 `PREVIEW_NOTICE_VERSION` 的版本号 **+1**（现在是 `'3'`，改成 `'4'` 即可） |
