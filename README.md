...
| **「到目前一共花了多少钱」这个数字** | `js/common.js` 最上面的 `SITE_COST` | 只改 `amount` 那一行（现在是 `¥59.78`），首页那条赞赏码、赞赏码弹窗、新人弹窗会一起变；顺便把 `checkedAt` 改成你核对这天的日期。数字真正生效的只有这一处；`index.html` 里那两处 `data-site-cost` 占位里也写了一个同样的数，那是**没脚本时的兜底**，换数字时顺手一起改 |
